...
//    [[contentView layer] setShadowOffset:CGSizeMake(10,10)];
//  //  [[contentView layer] setShadowPath:[UIBezierPath bezierPathWithRect:contentView.bounds].CGPath];
//    [[contentView layer] setShadowRadius:5];
//    [[contentView layer] setShadowOpacity:0.25];
//    [[contentView layer] setShadowColor:[UIColor blackColor].CGColor];
//    [[contentView layer] rasterizeShadow];
...
{	
    ...
    switch (type)
    {
        ...
        case iCarouselTypeCoverFlow2:
        {
            ...
            CGFloat spacing = [self valueForTransformOption:iCarouselTranformOptionSpacing withDefault:1/5.5f]; // should be ~ 1/scrollSpeed;
...
//            CGFloat z = fabsf(clampedOffset) * -itemWidth * 0.5f;
            CGFloat x = (clampedOffset * 1.0f * tilt + offset * spacing) * (itemWidth+170);
            //            CGFloat z = fabsf(clampedOffset) * -itemWidth * 0.75f;
            CGFloat z = fabsf(clampedOffset) * -(itemWidth -150) * 0.9f;
            ...
            else
            {
                ...
                return CATransform3DRotate(transform, -clampedOffset * M_PI_4 * tilt/5, 0.0f, 1.0f, 0.0f);
            }
        }
		case iCarouselTypeTimeMachine:
        case iCarouselTypeInvertedTimeMachine:
...
{
	if ([delegate respondsToSelector:@selector(touchesBegan)])
	{
		return [delegate touchesBegan];
	}
}

- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
	if ([delegate respondsToSelector:@selector(touchesEnded)])
	{
		return [delegate touchesEnded];
	}
}
@end